...
(function () {
  ...
  function loadGiscus() {
    ...
    // 기존 Giscus 스크립트가 있으면 제거 (게시글 변경 시 재로드)
    ...
    script.setAttribute("data-repo-id", "R_kgDOQecy-Q"); // ⚠️ 실제 값으로 교체 필요
    script.setAttribute("data-category", "General");
    script.setAttribute("data-category-id", "DIC_kwDOQecy-c4CzIyO"); // ⚠️ 실제 값으로 교체 필요
    // url 매핑: 전체 URL(쿼리 파라미터 포함)을 기반으로 각 게시글별 댓글 구분
  ...
  async function loadPost() {
    ...
    if (!filename) {
      if (contentEl) {
        contentEl.innerHTML =
          '<p>게시글을 찾을 수 없습니다. <a href="index.html">목록으로 돌아가기</a></p>';
      }
      return;
    }
